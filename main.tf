...
resource "aws_security_group" "master" {
  count                  = module.this.enabled && var.use_existing_additional_master_security_groups == false ? 1 : 0
  revoke_rules_on_delete = true
  vpc_id                 = var.vpc_id
...
resource "aws_security_group_rule" "master_ingress_security_groups" {
  count                    = module.this.enabled && var.use_existing_additional_master_security_groups == false ? length(var.master_allowed_security_groups) : 0
...
resource "aws_security_group_rule" "master_ingress_cidr_blocks" {
  count             = module.this.enabled && length(var.master_allowed_cidr_blocks) > 0 && var.use_existing_additional_master_security_groups == false ? 1 : 0
...
resource "aws_security_group" "slave" {
  count                  = module.this.enabled && var.use_existing_additional_slave_security_groups == false ? 1 : 0
  revoke_rules_on_delete = true
  vpc_id                 = var.vpc_id
...
resource "aws_security_group_rule" "slave_ingress_security_groups" {
  count                    = module.this.enabled && var.use_existing_additional_slave_security_groups == false ? length(var.slave_allowed_security_groups) : 0
  description              = "Allow inbound traffic from Security Groups"
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
...
resource "aws_security_group_rule" "slave_ingress_cidr_blocks" {
  count             = module.this.enabled && length(var.slave_allowed_cidr_blocks) > 0 && var.use_existing_additional_slave_security_groups == false ? 1 : 0
  description       = "Allow inbound traffic from CIDR blocks"
  type              = "ingress"
  from_port         = 0
  to_port           = 65535
...
resource "aws_emr_cluster" "default" {
  ...
  ec2_attributes {
    ...
    additional_master_security_groups = var.use_existing_additional_master_security_groups == false ? join("", aws_security_group.master.*.id) : join(",", var.additional_master_security_groups)
    additional_slave_security_groups  = var.use_existing_additional_slave_security_groups == false ? join("", aws_security_group.slave.*.id) : join(",", var.additional_slave_security_groups)
  }
